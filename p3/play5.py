...
def wall_follow(scan, fm, adir, follow_side, left_close, right_close):
...
# P1: "장애물이 가까운데 회피 회전을 안 하고 있다"를 감지하기 위한 임계값
# (전방 fm 또는 측방 left_close/right_close 중 하나라도 STUCK_DIST 이내인데
#  회전량 |w|가 STUCK_W_THRESH 미만인 상태가 STUCK_TIMEOUT_SEC 이상 지속되면 LOOP_ESCAPE)
STUCK_DIST        = 30.0
STUCK_W_THRESH    = 0.15
...
# P1 Watchdog   : 아래 세 조건 중 하나라도 만족하면 LOOP_ESCAPE로 강제 전환.
#                 1) |회전 누적| > FULL_LOOP_THRESH   (한쪽으로 계속 도는 "진짜 맴돌이")
#                 2) 탐색 경과시간 > MISSION_TIMEOUT_SEC  (같은 색 타겟을 너무 오래 못 찾음)
#                 3) 전방/측방 30cm(STUCK_DIST) 이내 장애물이 있는데 회전(|w|)을 거의 안 한 채
#                    STUCK_TIMEOUT_SEC(3초) 이상 지속  (장애물을 감지하고도 회피하지 않음)
#                 탐색 상태(WALL_SEARCH/APPROACH/FOLLOW)에서만 감시하고, TRACK/FORWARD/PARKING처럼
#                 "진행 중"인 상태에서는 누적/타이머를 멈춘다(=watchdog 일시정지) → 오작동 개입 방지.
#                 조건 1은 부호를 유지(abs 제거)하여 좌우로 흔들리는 정상 벽 추종은 상쇄되고,
#                 한쪽으로 계속 도는 경우만 임계치에 걸리도록 함.
...
escape_side   = "L"    # ★★ 신규: LOOP_ESCAPE 진입 순간의 follow_side를 그대로 기억(스냅샷).
                        #   진입 후에는 follow_side가 다른 이유(타겟 소실 등)로 바뀌어도
                        #   탈출 회전 방향은 "맴돌던 그 벽" 기준으로 고정하기 위함.
loop_accum    = 0.0    # P1 Watchdog: 탐색 상태에서 누적된 w*dt (부호 유지, 단위: rad).
                        #   좌우로 흔들리며 전진하는 정상 주행은 상쇄되고, 한 방향으로
                        #   계속 도는 경우에만 절댓값이 임계치에 도달한다.
loop_t_prev   = time.time()

stuck_t       = 0.0    # P1 Watchdog: 장애물이 가까운데(<STUCK_DIST) 회전을 거의 안 한 채(<STUCK_W_THRESH)
                        #   누적된 시간(초). 탐색 상태에서만 누적되며, 조건이 깨지면 즉시 0으로 리셋된다.
...
try:
    while True:
        ...
        cv2.putText(frame, f"STATE:{state} LOOP:{math.degrees(abs(loop_accum)):.0f}/{math.degrees(FULL_LOOP_THRESH):.0f}deg STUCK:{stuck_t:.1f}/{STUCK_TIMEOUT_SEC:.0f}s",
                    (10, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)

        # ══ P1: Watchdog (회전 누적 / 타임아웃 / 근접 미회피) ════════════════
        # 탐색 상태에서만 누적하고, TRACK처럼 목표를 향해 "진행 중"일 때는 누적을 멈춘다.
        if state in EXPLORE_STATES:
            loop_accum += last_sent_w * dt  # 부호 유지: 좌우로 흔들리는 정상 추종은 상쇄, 한쪽으로 계속 도는 것만 누적

            # 조건 3: 전방/측방 30cm 이내에 장애물이 있는데 회전을 거의 안 한 채 5초 이상 지속
            near_obstacle = (fm < STUCK_DIST) or (left_close < STUCK_DIST) or (right_close < STUCK_DIST)
            not_avoiding  = abs(last_sent_w) < STUCK_W_THRESH
            if near_obstacle and not_avoiding:
                stuck_t += dt
            else:
                stuck_t = 0.0

            if (now_t - mission_start_t > MISSION_TIMEOUT_SEC) or (abs(loop_accum) > FULL_LOOP_THRESH) or (stuck_t > STUCK_TIMEOUT_SEC):
                state = "LOOP_ESCAPE"
                escape_t = now_t
                escape_side = follow_side   # ★★ 신규: 맴돌 때 추종 중이던 벽을 기억
                loop_accum = 0.0
                stuck_t = 0.0
                detect_count = 0
                cv2.imshow("f", frame); cv2.waitKey(1); continue
        ...
        if state == "WALL_SEARCH":
            ...
            v, w = wall_follow(scan, fm, adir, follow_side, left_close, right_close)
            send_cmd(v, w)

        elif state == "LOOP_ESCAPE":
            # 1) 잠시 후진 → 2) 추종하던 벽의 반대 방향으로 회전해 새 방향 모색 → 3) WALL_SEARCH로 복귀
            #   escape_side == "L"(왼쪽 벽 추종 중 맴돔) -> 오른쪽으로 회전(w>0)
            #   escape_side == "R"(오른쪽 벽 추종 중 맴돔) -> 왼쪽으로 회전(w<0)
            escape_w_sign = 1 if escape_side == "L" else -1
            ...
            if elapsed_esc < LOOP_ESCAPE_SEC:
                ...
                send_cmd(0.0, escape_w_sign * WALL_SEARCH_W * 1.2)
            else:
                state = "WALL_SEARCH"
                mission_start_t = now_t
                loop_accum = 0.0
