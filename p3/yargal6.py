    ...
    while True:
        ...
        # ══ PARK 모드 ════════════════════════════════════════════
            ...
            if lidar_state == "WALL_SEARCH":
                ...
                # 정면이든 측면이든, 더 가까운 쪽에서 먼저 벽이 잡히면 바로 접근 단계로 전환
                if fm < WALL_SCAN_DIST or side_min_d < WALL_SCAN_DIST:
                    follow_side = "L" if l <= r else "R"
                    lidar_state = "WALL_APPROACH"
                    print(f"[LIDAR] 벽 감지 fm:{fm:.0f}cm L:{l:.0f}cm R:{r:.0f}cm, follow_side={follow_side} → 접근 시작")
                else:
                    send_cmd(0.0, WALL_SEARCH_W)
                    cv2.putText(frame, f"LIDAR-WALL-SEARCH fm:{fm:.0f} L:{l:.0f} R:{r:.0f}",
                                (10, 25), 0, 0.5, (0, 255, 0), 1)
