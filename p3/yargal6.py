...
# ── ODOMETRY (위치 추정) ──────────────────────────────────────────────
ROBOT_X, ROBOT_Y, ROBOT_TH = 0.0, 0.0, 0.0
last_odom_t = time.time()

BOUND_X     = 1.0    # 좌우 ±1m (시작점 기준)
BOUND_Y_MAX = 2.0    # 전방 2m
BOUND_Y_MIN = -0.1   # 살짝의 후방 여유
MARGIN      = 0.15   # 벽 감지 여유 거리

def update_odom(v, w):
    global ROBOT_X, ROBOT_Y, ROBOT_TH, last_odom_t
    now = time.time()
    dt  = now - last_odom_t
    last_odom_t = now
    ROBOT_X  += v * math.cos(ROBOT_TH) * dt
    ROBOT_Y  += v * math.sin(ROBOT_TH) * dt
    ROBOT_TH += w * dt
...
last_odom_t = time.time()  # 초기화 지연 보정
...
try:
    while True:
        ...
        if mode == "LIDAR":
            ...
            cv2.putText(frame, f"POS: ({ROBOT_X:.2f},{ROBOT_Y:.2f}) TH:{math.degrees(ROBOT_TH):.0f}",
                        (10, 45), 0, 0.5, (255, 255, 255), 1)
