...
WALL_TURN_V   = 0.10
WALL_LOST_W   = 0.5
WALL_SEARCH_W = 1.1     # 벽 탐색 회전 각속도 (나선의 각속도 성분)

# ── SPIRAL SEARCH (테오도로스/아르키메데스 나선형 탐색) ──────────────
# v=0 제자리 회전으로는 주변에 아무것도 없을 때 영원히 못 벗어나므로,
# 회전하면서 시간에 따라 v를 점점 키워 나선을 그리며 바깥으로 퍼져나간다.
SPIRAL_V_START  = 0.0
SPIRAL_V_MAX    = 0.18
SPIRAL_V_GROWTH = 0.015   # 초당 전진속도 증가량 (m/s 단위 가정)
SPIRAL_W        = WALL_SEARCH_W

# WALL_SEARCH 상태에 "처음 진입한 시각"을 기록해 나선 반경을 계산한다.
# lidar_state / park_state 어느 쪽이든 WALL_SEARCH로 "새로" 들어올 때마다 리셋해야 한다.
spiral_start_t = None

def spiral_search_cmd(fm_now):
    """
    나선형 탐색 명령 (v, w)을 계산해서 반환.
    spiral_start_t가 None이면 지금 이 순간을 시작점으로 기록한다.
    """
    global spiral_start_t
    if spiral_start_t is None:
        spiral_start_t = time.time()

    elapsed = time.time() - spiral_start_t
    v = min(SPIRAL_V_START + SPIRAL_V_GROWTH * elapsed, SPIRAL_V_MAX)

    # 안전장치: 나선이 커지다가 갑자기 장애물에 가까워지면 즉시 속도를 줄인다.
    if fm_now < THRESH_STOP:
        v = 0.05
    elif fm_now < THRESH_TURN:
        v = min(v, 0.08)
    ...
    while True:
        ...
        if mode == "LIDAR":
            ...
            if lidar_state == "WALL_SEARCH":
                if fm < WALL_SCAN_DIST:
                    ...
                    v, w = spiral_search_cmd(fm)
                    send_cmd(v, w)
                    cv2.putText(frame, f"LIDAR-WALL-SEARCH(spiral) fm:{fm:.0f} v:{v:.2f}",
        ...
        # ══ PARK 모드 ════════════════════════════════════════════
            ...
            # ── 3-A. 벽 탐색 중 나선형 회전 (WALL_SEARCH) ────────
                ...
                # 회전+전진(나선)하면서 정면(front_min) 기준으로 벽 감지
                if fm < WALL_SCAN_DIST:
                    l = side_dist(scan, "L")
                    r = side_dist(scan, "R")
                    follow_side = "L" if l <= r else "R"
                ...
                # 아직 아무것도 없음 → 나선형으로 점점 퍼지며 탐색
                v, w = spiral_search_cmd(fm)
                send_cmd(v, w)
                cv2.putText(frame, f"WALL-SEARCH(spiral) [{target}] fm:{fm:.0f} v:{v:.2f}",
                            (10, 25), 0, 0.5, (0, 255, 0), 1)
