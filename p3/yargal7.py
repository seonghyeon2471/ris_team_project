...
WALL_APPROACH_V   = 0.26
WALL_KP           = 0.016
WALL_V            = 0.28
WALL_TURN_V       = 0.13
WALL_LOST_W       = 0.55
WALL_SEARCH_W     = 1.6
WALL_LOST_DIST    = 80.0   # 이 거리 이상이면 현재 벽 "소실" 판정
WALL_SWITCH_MARGIN = 0.75  # 반대쪽 벽이 현재 벽 거리의 이 비율 이하면 전환

def wall_follow(scan, fm, adir, follow_side):
    """
    양쪽 벽을 모두 모니터링하며 더 가까운 쪽으로 자동 전환.
      - 현재 벽이 WALL_LOST_DIST 이상 → 반대쪽 벽으로 전환
      - 반대쪽 벽이 현재 벽의 WALL_SWITCH_MARGIN 배 이하 & 합리적 범위 → 전환
    반환: (v, w, new_follow_side)
    """
    sd_cur = side_dist(scan, follow_side)
    opp    = "R" if follow_side == "L" else "L"
    sd_opp = side_dist(scan, opp)

    # ── 벽 전환 판단 ──────────────────────────────────────────────────
    new_side = follow_side
    if sd_cur > WALL_LOST_DIST:
        new_side = opp
        print(f"[WALL] {follow_side} 벽 소실({sd_cur:.0f}cm) → {opp} 전환")
    elif sd_opp < sd_cur * WALL_SWITCH_MARGIN and sd_opp < WALL_TARGET * 2.5:
        new_side = opp
        print(f"[WALL] {opp} 벽이 더 가깝다({sd_opp:.0f} vs {sd_cur:.0f}cm) → {opp} 전환")

    follow_side = new_side
    ...
    if fm < THRESH_STOP:
        return (0.08, adir * 1.1, follow_side)
    if fm < THRESH_TURN:
        return (WALL_TURN_V, adir * 0.85, follow_side)

    if left_close < THRESH_STOP:
        return (WALL_V * 0.7, -0.7, follow_side)
    if right_close < THRESH_STOP:
        return (WALL_V * 0.7,  0.7, follow_side)

    if sd > WALL_TARGET * 2.0:
        return (WALL_V * 0.7, sign * WALL_LOST_W, follow_side)
    ...
    return (v, w, follow_side)

# ── LIDAR 우선순위 판단 함수 ──────────────────────────────────────────
def lidar_priority(scan, fm, adir):
    if fm < THRESH_STOP:
        return True, 0.06, adir * 1.2, f"LIDAR-EMRG fm:{fm:.0f}"
    if fm < THRESH_TURN:
        return True, 0.10, adir * 0.9, f"LIDAR-AVOID fm:{fm:.0f}"
    ...
    while True:
        ...
        if mode == "LIDAR":
            ...
            # ── B. 벽으로 접근 중 (WALL_APPROACH) ─────────────────
                ...
                # 양쪽 모두 체크: 어느 쪽이든 목표 거리 이내면 WALL_FOLLOW 진입
                sd_l = side_dist(scan, "L")
                sd_r = side_dist(scan, "R")
                if sd_l <= WALL_TARGET * 1.3 or sd_r <= WALL_TARGET * 1.3:
                    # 더 가까운 쪽을 follow_side로 확정
                    follow_side = "L" if sd_l <= sd_r else "R"
                    lidar_state = "WALL_FOLLOW"
                    print(f"[LIDAR] 벽 도달 L:{sd_l:.0f} R:{sd_r:.0f}cm → {follow_side} wall-following 시작")
                else:
                    sign = 1 if follow_side == "L" else -1
                    if fm < THRESH_SLOW:
                        blend = float(np.clip(
                    ...
                    cv2.putText(frame, f"LIDAR-WALL-APPROACH {follow_side}:{sd_disp:.0f}cm",
                                (10, 25), 0, 0.5, (0, 200, 0), 1)

            # ── C. wall-following으로 탐색 (WALL_FOLLOW) ─────────
            elif lidar_state == "WALL_FOLLOW":
                v, w, follow_side = wall_follow(scan, fm, adir, follow_side)
                send_cmd(v, w)
                sd_disp = side_dist(scan, follow_side)
        ...
        # ══ PARK 모드 ════════════════════════════════════════════════
            ...
            # ── 3-B. 벽으로 접근 중 (WALL_APPROACH) ───────────────
                ...
                if sd_l <= WALL_TARGET * 1.3 or sd_r <= WALL_TARGET * 1.3:
                    ...
                    print(f"벽 도달 L:{sd_l:.0f} R:{sd_r:.0f}cm → {follow_side} wall-following 시작")
                    continue
                ...
                send_cmd(v, w)
                sd_disp = side_dist(scan, follow_side)
                cv2.putText(frame, f"WALL-APPROACH [{target}] {follow_side}:{sd_disp:.0f}cm",
                            (10, 25), 0, 0.5, (0, 200, 0), 1)
