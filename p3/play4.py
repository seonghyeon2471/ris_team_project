...
#                 누적 시 부호를 유지(abs 제거)하여 좌우로 흔들리는 정상 벽 추종은 상쇄되고,
#                 한쪽으로 계속 도는 "진짜 맴돌이"만 임계치에 걸리도록 함.
...
try:
    while True:
        ...
        cv2.putText(frame, f"STATE:{state} LOOP:{math.degrees(abs(loop_accum)):.0f}/{math.degrees(FULL_LOOP_THRESH):.0f}deg",
        ...
        if state in EXPLORE_STATES:
            ...
            if (now_t - mission_start_t > MISSION_TIMEOUT_SEC) or (abs(loop_accum) > FULL_LOOP_THRESH):
                state = "LOOP_ESCAPE"
                escape_t = now_t
