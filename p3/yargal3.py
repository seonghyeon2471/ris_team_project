...
# ── ODOMETRY FROM ARDUINO ─────────────────────────────────────────────
robot_dist = 0.0
enc_l = 0
enc_r = 0

def arduino_rx_loop():
    global robot_dist, enc_l, enc_r
...
try:
    while True:
        ...
        obs_mask = obstacle_mask(frame, hsv)

        obs_cnts, _ = cv2.findContours(
            obs_mask,
            cv2.RETR_EXTERNAL,
        ...
        if obs_cnts:
            ...
            obs = max(obs_cnts, key=cv2.contourArea)

            if cv2.contourArea(obs) > 800:

                x,y,w,h = cv2.boundingRect(obs)

                obs_cx = x + w//2

                obs_err_x = obs_cx - cx_mid
