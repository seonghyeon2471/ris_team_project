...
# ★ 수정(실측 확인): adir==1 은 "오른쪽이 더 트임" 이므로 오른쪽 벽 추종("R")이 맞음 (기존: 반대로 매핑되어 있었음)
...
def wall_follow(scan, fm, adir, follow_side):
    ...
    # ★ 수정(실측 확인): w>0 은 실제로 R(오른쪽) 회전이므로, follow_side="L"(왼쪽 벽 추종)일 때
    #   왼쪽 벽에서 멀어지면(err>0) 왼쪽(w<0)으로 돌아야 함 -> sign 부호 반전 (기존: 1 if L else -1)
    ...
    # ★ 수정(실측 확인): w>0=R, w<0=L 좌표계. 왼쪽이 너무 가까우면 오른쪽(w>0)으로,
    #   오른쪽이 너무 가까우면 왼쪽(w<0)으로 피해야 함 (기존: 부호가 반대로 되어 있었음)
    if left_close < SIDE_STOP:
        return (WALL_V * 0.7,  0.7)
    if right_close < SIDE_STOP:
        return (WALL_V * 0.7, -0.7)

    # ★ 수정(실측 확인): w>0=R, w<0=L 좌표계에 맞게 부호 반전.
    #   왼쪽이 더 가까우면(err_center<0) 오른쪽으로(양의 w), 오른쪽이 더 가까우면(err_center>0) 왼쪽으로(음의 w)
...
# ★ 신규: 색지가 "보이다가 사라지는 순간"을 감지하기 위한 직전 프레임 found 상태
was_found     = False
...
try:
    while True:
        ...
        # ★ 신규: 색지가 "보이다가 사라지는 순간"을 감지해서 그 직전 위치(last_seen_x) 기준으로
        #   follow_side를 즉시 갱신. WALL_FOLLOW/WALL_APPROACH 중에만 의미가 있지만,
        #   다른 상태에서 갱신돼도 무해하므로 매 프레임 체크.
        #   (사라진 쪽=색지가 마지막으로 보였던 쪽 벽을 그대로 추종 -> 같은 follow_side 로직 재사용)
