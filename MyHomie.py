...
# ── LIDAR 모드 조향 구간 (화면 W=320 기준) ────────────────────────────
# 양옆 30%: 0~96px, 224~320px  → 카메라 조향
# 중앙 40%: 96~224px           → 라이다 조향
CAM_ZONE_W  = int(320 * 0.30)   # 96px
MID_X_LEFT  = CAM_ZONE_W        # 96
...
try:
    while True:
        ...
        # LIDAR 모드 조향 구간 시각화
        cv2.line(frame, (MID_X_LEFT,  0), (MID_X_LEFT,  H - 1), (200, 200, 0), 1)
        cv2.line(frame, (MID_X_RIGHT, 0), (MID_X_RIGHT, H - 1), (200, 200, 0), 1)
        ...
        if mode == "LIDAR":
            ...
            if found and cx_obj >= 0:
                # ── 색상 발견 시: 센트로이드 위치로 조향 스위칭 ──
                in_side_zone = (cx_obj < MID_X_LEFT) or (cx_obj > MID_X_RIGHT)

                if in_side_zone:
                    # 양옆 30%: 카메라 조향으로 중앙 맞추기
                    w = cam_w(cx_obj - cx_mid)
                    if fm < THRESH_STOP:
                        v = 0.09
                    elif fm < THRESH_TURN:
                        v = 0.13
                ...
                send_cmd(v, w)
                cv2.putText(frame, f"LIDAR+{steer_label}: {target}", (10, 25),
                            0, 0.5, draw, 1)
        ...
        # ══ PARK 모드 ════════════════════════════════════════════
            ...
            if park_state == "FORWARD":
                ...
                cv2.putText(frame, f"TRACK+{steer_label}: {target}", (10, 25), 0, 0.6, draw, 1)
