...
odom_y     = 0.0
odom_theta = 0.0
odom_last_time = time.time()

def update_odometry(v, w, dt):
    global odom_x, odom_y, odom_theta
    odom_theta += w * dt
    odom_x     += v * math.cos(odom_theta) * dt
    odom_y     += v * math.sin(odom_theta) * dt

# =========================================
# VISIT MAP
# =========================================
GRID_RESOLUTION = 20.0   # 격자 크기 (cm)
visit_map = {}           # (gx, gy) → 방문 횟수

def get_grid(x_cm, y_cm):
    return (int(x_cm / GRID_RESOLUTION), int(y_cm / GRID_RESOLUTION))

def record_visit():
    gx, gy = get_grid(odom_x * 100, odom_y * 100)  # m → cm
    visit_map[(gx, gy)] = visit_map.get((gx, gy), 0) + 1

def get_visit_count_at_angle(angle_deg, dist_cm):
    """특정 방향 dist_cm 앞 격자의 방문 횟수 반환"""
    rad  = math.radians(odom_theta + math.radians(angle_deg))
    tx   = odom_x * 100 + dist_cm * math.cos(rad)
    ty   = odom_y * 100 + dist_cm * math.sin(rad)
    return visit_map.get(get_grid(tx, ty), 0)

EXPLORE_WEIGHT = 15.0   # 미방문 방향 선호 강도 (튜닝 포인트)
...
def score_gap(gap, proc_dists, angles):
    ...
    # 갭 중심 방향의 방문 횟수
    visits = get_visit_count_at_angle(center_angle, avg_dist)

    # 방문 많을수록 페널티
    explore_bonus = -EXPLORE_WEIGHT * math.log1p(visits)

    return width * 0.5 + avg_dist * 1.2 - abs(center_angle) * 0.4 + explore_bonus
...
try:
    while True:
        # while True 안, 스캔 처리 후:
        now = time.time()
        dt  = now - odom_last_time
        odom_last_time = now
        update_odometry(v_cmd, w_cmd, dt)
        record_visit()
