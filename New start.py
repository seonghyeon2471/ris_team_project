...
def nearest_obstacle_angle():  # [수정] 매개변수 제거
    return int(np.argmin(scan_data))

def nearest_obstacle_dist():  # [수정] 매개변수 제거
...
def wall_follow(scan, fm, adir):  # [수정] scan 매개변수 유지하지만 내부에서는 사용하지 않음
    ld = left_dist(scan)
    ...
    if ld > WALL_TARGET * 2.0:
        nearest = nearest_obstacle_angle()  # [수정] 매개변수 없이
...
try:
    while True:
        ...
        if mode == "LIDAR":
            ...
            nearest = nearest_obstacle_angle()  # [수정] 매개변수 없이
            nd = nearest_obstacle_dist()  # [수정] 매개변수 없이
            ...
            if park_state == "FORWARD":
                ...
                if front_nearest < WALL_SCAN_DIST:
                    ...
                    continue
                nearest_sa = nearest_obstacle_angle()  # [수정] 매개변수 없이
                nd_sa = nearest_obstacle_dist()  # [수정] 매개변수 없이
                if nd_sa < 80.0:
                ...
                if time.time() - ws_start_t > 6.0:
                    ...
                    time.sleep(0.8)
                    ws_start_t = time.time()
                cv2.putText(frame, f"WALL-SEARCH [{target}] nd={nd_sa:.0f}cm", (10, 25), 0, 0.5, (0, 255, 0), 1)
                ...
                if fm < THRESH_10:
                    ...
                    nearest_wa = nearest_obstacle_angle()  # [수정] 매개변수 없이
                    err_a = nearest_wa if nearest_wa <= 180 else nearest_wa - 360
                    w_a = float(np.clip(-err_a / 120.0 * 0.6, -0.6, 0.6))
                    v, w = WALL_APPROACH_V, w_a
                send_cmd(v, w)
                cv2.putText(frame, f"WALL-APPROACH [{target}] nd={nd:.0f}cm", (10, 25), 0, 0.45, (0, 200, 0), 1)
                ...
                cv2.putText(frame, f"WALL-FOLLOW [{target}] L:{ld_disp:.0f}cm rot:{accum_deg:.0f}deg", (10, 25), 0, 0.45, (0, 255, 0), 1)
